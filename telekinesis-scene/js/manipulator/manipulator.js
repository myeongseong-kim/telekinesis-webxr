...
export function decomposeSwingTwist(q, axis, swing, twist) {
  let dir = new THREE.Vector3(q.x, q.y, q.z);
  let proj = dir.clone().projectOnVector(axis);

  twist.set(proj.x, proj.y, proj.z, q.w).normalize();
  swing.copy(q).multiply(twist.clone().invert());
